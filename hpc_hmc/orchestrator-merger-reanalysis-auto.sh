...
do
	...
	if [ $i == 1 ]; then
		m1_job_id=$(sbatch --parsable --ntasks=${days} merger-domain-reanalysis.slurm ${calendar})
	elif [ -v m1_job_id ]; then
		m1_job_id=$(sbatch --parsable --ntasks=${days} --dependency=afterok:$m1_job_id merger-domain-reanalysis.slurm ${calendar})			
	fi
	echo " ==> Merger by domain scheduled with job id "${m1_job_id}"."

	echo " ==> Scheduling merger by time with calendar file "${calendar}" ..."
	if [ $i == 1 ]; then
		m2_job_id=$(sbatch --parsable --ntasks=1 --dependency=afterok:$m1_job_id merger-time-reanalysis.slurm ${time_start} ${time_end})
	elif [ -v m2_job_id ]; then
		m2_job_id=$(sbatch --parsable --ntasks=1 --dependency=afterok:$m1_job_id,$m2_job_id merger-time-reanalysis.slurm ${time_start} ${time_end})
	fi
...
if [ -e "$file" ]; then
  ...
  scheduler_job_id=$(sbatch --parsable --dependency=afterok:$(IFS=,; echo "${aiJobIds[*]}") scheduler-merger-reanalysis.slurm ${id})
fi

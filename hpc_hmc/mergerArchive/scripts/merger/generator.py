...
def generateString(oTimeStart, oTimeEnd, iDays):
    """
    Support function to generate lines for the simulation setup.
    """
    return f"{oTimeStart.strftime('%Y-%m-%dT%H:%M')}     {oTimeEnd.strftime('%Y-%m-%dT%H:%M')}     {iDays}"
...
def generatePeriods(sFileName: str, startTime: str):
    ...
    try:
        ...
        with open(sFileName, "w", encoding="utf-8") as f:
            f.write("TimeStart            TimeEnd              Days\n")
            f.write(generateString(oStart, oEnd, iDays))
            f.write("\n")
        return oEnd

...
do
	...
	do
		...
		if [ $i == 1 ]; then
			s3m1_job_id=$(sbatch --parsable --time=${s3m_max_time} s3m-rcp85.slurm ${domain_name} ${s3m_restart_flag} ${s3m_terrdata_flag} "${s3m_time_restart}" "${s3m_time_start}" "${s3m_time_end}" ${s3m_time_period})
		elif [ -v s3m1_job_id ]; then
			s3m1_job_id=$(sbatch --parsable --time=${s3m_max_time} --dependency=afterok:$s3m1_job_id s3m-rcp85.slurm ${domain_name} ${s3m_restart_flag} ${s3m_terrdata_flag} "${s3m_time_restart}" "${s3m_time_start}" "${s3m_time_end}" ${s3m_time_period})
		fi

...
do
        ...
        do
            ...
            s3m_terrdata_flag=$(cat ${periods}|sed '1d'|awk -v var1=${i} 'NR==var1{print $7}')
                ...
                echo " ==> Scheduling converter rcp85 execution for domain: "${domain_name}" "${conv_time_start}" "${conv_time_end}" ..."
                if [ $i == 1 ]; then
...
if [ -e "$file" ]; then
  ...
  scheduler_job_id=$(sbatch --parsable --dependency=afterok:$(IFS=,; echo "${aiJobIds[*]}") scheduler-rcp85.slurm)
fi
